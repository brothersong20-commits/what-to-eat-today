...
-- 1. 식당 5개
insert into public.restaurants (id, name, category, area, address, walking_minutes, capacity, menus_text, note, active) values
  ('R001', '청담옥',     '한식', '미지정', '강남구 테헤란로 123',  5, '룸 4~12인 / 단체 30명', '한우갈비살(45000)/평양냉면(12000)/육회(28000)',         '주차 가능, 흡연실 있음', true),
  ('R002', '스시조',     '일식', '미지정', '강남구 역삼동 456',    8, '단체석 가능 / 룸 1개',  '런치오마카세(60000)/디너오마카세(120000)',              '예약 필수',              true),
  ('R003', '딘타이펑',   '중식', '미지정', '강남구 강남대로 789',  6, '단체 20명 가능',        '샤오롱바오(13000)/볶음밥(11000)/마파두부(16000)',       '대기 길음',              true),
  ('R004', '에이오씨',   '양식', '미지정', '강남구 도산대로 12',  10, '룸 8인 / 홀 단체 가능', '파스타런치(22000)/스테이크(48000)/하우스와인(9000)',    '분위기 좋음',            true),
  ('R005', '교대이층집', '고기', '미지정', '서초구 서초대로 33',  12, '단체 40명 / 룸 다수',   '한우꽃등심(58000)/된장찌개(8000)/공기밥(2000)',         '회식 단골',              true)
on conflict (id) do nothing;

-- 기존 식당(라이브 데이터 포함) area 일괄 채움. 이후 관리자에서 식당별 실제 지역 지정.
update public.restaurants set area = '미지정' where area is null;
